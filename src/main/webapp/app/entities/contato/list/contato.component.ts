...
import { EntityArrayResponseType, ContatoService } from '../service/contato.service';
...
export class ContatoComponent implements OnInit {
  ...
  trackId = (_index: number, item: IContato): number => this.contatoService.getContatoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.contatoes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IContato[] | null): IContato[] {
    // If there is previus link, data is a infinite scroll pagination content.
    if ('prev' in this.links) {
      const contatoesNew = this.contatoes ?? [];
      if (data) {
        for (const d of data) {
          if (contatoesNew.map(op => op.id).indexOf(d.id) === -1) {
            contatoesNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.contatoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
